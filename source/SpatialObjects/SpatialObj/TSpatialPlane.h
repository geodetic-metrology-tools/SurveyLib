...
#ifndef SU_SPATIALPlANE
#define SU_SPATIALPlANE
...
public:
	...
	//@{
		...
		TSpatialPlane(const TSpatialPointName firstPointName, const TSpatialPointName secondPointName);

		/*!Constructor for a vertical plane
		\param firstPointName the name of the Plane's first point
		\param angle the line'orientation*/
		TSpatialPlane(const TSpatialPointName firstPointName, const TAngle angle);
	
		//!Copy constructor
		TSpatialPlane(const TSpatialPlane& source);
	...
	//@{
		//!Equality operator
		virtual bool					operator==(const TSpatialPlane& right) const;
		//!operator =
		virtual void					operator=(const TSpatialPlane& right);
