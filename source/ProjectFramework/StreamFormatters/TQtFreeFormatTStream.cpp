// TQtFreeFormatTStream.h
//
//
// Concrete class defining the free format file syntax,
// for example the file as used in CSGeo, and Geotran.
//
// Patterns:
// A concrete decorator class of a QTextStream.
// 
// Copyright 1999,2000 CERN, EST/SU. All rights reserved.
//////////////////////////////////////////////////////////////////////


////////////////////////////////////////////////////////////////
//For ROOT//////////////////////////////////////////////////////
//#include	"TROOT.h"
//
// Other forward declarations
#include  "TAngleFilter.h"
#include  "TLengthFilter.h"
#include  "TSpatialPositionFilter.h"
#include  "TQtFreeFormatTStream.h"
#include  "TSpatialPoint.h"
#include  "TSpatialPointName.h"
#include  "TSpatialPosition.h"
#include  "QTextStream.h"
//////////////////////////////////////////////////////////////////////


//ClassImp(TQtFreeFormatTStream)


//////////////////////////////////////////////////////////////////////
// Construction/Destruction
//////////////////////////////////////////////////////////////////////

TQtFreeFormatTStream::TQtFreeFormatTStream( QTextStream  *stream ) : TAQtStreamFormatter( stream )
{

	//fCoordToken;
	//fCoorSys;
}

TQtFreeFormatTStream::~TQtFreeFormatTStream()
{
	delete fStream;
}


//////////////////////////////////////////////////////////////////////
// Member Functions
//////////////////////////////////////////////////////////////////////

/////////////////////////////////////////////
// returns the string used to separate fields
/////////////////////////////////////////////
QString TQtFreeFormatTStream::separator() const{

	QString sep(" ");
	return sep;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator>>( TAngle &angle )
{	//input an angle from the text stream
	
	// pass on the request to the angles filter Strategy
	fAngFilter->input(*fStream, angle);

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator>>( TLength &length )
{	//input a length from the text stream
	
	// pass on the request to the lengths filter Strategy
	fLenFilter->input(*fStream, length);

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator>>( TSpatialPointName &ptName )
{	//input a spatial point name from the text stream
	
	//char  cStr[20];
	QString  string;

	// get the name of the point as a single string from the text stream
	// and set the spatial point name
	this->TAQtStreamFormatter::operator>>( string );
	ptName.setName( string.latin1() );

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator>>( TSpatialPosition &position )
{	//input a spatial position from the text stream
	
	// pass on the request to the coordinate system filter Strategy
	fPosFilter->input(*this, position);

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator>>( TSpatialPoint &point )
{	

	TSpatialPointName  name;
	TSpatialPosition*	position = new TSpatialPosition( fRefFrame );
	
	this->skipCommentLines();

	// get the point name
	*this >> name;

	// get the point posiiton
	*this >> *position;
	
	point.setName( name );
	point.setPosition( position );
	
	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator<<( const TAngle &angle )
{	//output an angle to the text stream
	
	// pass on the request to the angles filter Strategy
	fAngFilter->output(*fStream, angle);

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator<<( const TLength &length )
{	//output a length to the text stream
	
	// pass on the request to the lengths filter Strategy
	fLenFilter->output(*fStream, length);

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator<<( const TSpatialPointName &ptName )
{	//output a spatial point name to the text stream
	
	int  width = this->width(); // the width assigned to the output

	// extract the name of the point as a single string and output to the
	// text stream
	this->width( width );
	if( ptName.composite() )
	{
		this->TAQtStreamFormatter::operator<<( (ptName.getDBName()).c_str() );
	}
	else
	{
		this->TAQtStreamFormatter::operator<<( (ptName.getName()).c_str() );
	}

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator<<( const TSpatialPosition &position )
{	//input a spatial position from the text stream
	
	// pass on the request to the coordinate system filter Strategy
	fPosFilter->output(*this, position);

	return *this;
}


TAQtStreamFormatter  &TQtFreeFormatTStream::operator<<( const TSpatialPoint &point )
{	
	// Get the width and precision for the point output
	// The assigned width is divided evenly between the point name and the
	// three point coordinates
	int  width = this->width() - 3; // reduce width by 3 to account for spaces
	int  precision = this->precision();
	int  remainder = width % 4;

	//divide the width by four to determine the amount to assign to the name,
	// and the three coordinates
	width /= 4;
	
	
	// output the point name
	// assign the additional width to the point name
	this->setf( QTextStream::left );
	this->width( width + remainder ); 
	*this << point.getName(); 
	this->TAQtStreamFormatter::operator<< ( ' ' );
	this->unsetf( QTextStream::left );

	// output the position
	//if ( point.positionKnown() )
	{
		this->width( 3*width );
		this->precision( precision );
		*this << point.getPosition( getReferenceFrame() );
	}
	
	return *this;
}

//////////////////////////////////////////////////////////////////////


void  TQtFreeFormatTStream::skipCommentLines()
{	// method to skip over the white space and comment lines
	// from the input stream

	// skip over the white space in the file
	this->skipWhiteSpace();
	
	// loop through all unrequired text
	while( '#' == this->peek() ) // # character indicates a comment line
	{
		// read the comment line and skip any more white space
		this->readLine();
		this->skipWhiteSpace();
	}

	
	return;
}



//////////////////////////////////////////////////////////////////////


