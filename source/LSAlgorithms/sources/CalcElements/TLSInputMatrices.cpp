...
bool TLSInputMatrices::setFirstDgnMtrxElement(MatrixIndex row, MatrixIndex column, TReal coeff)
{
	...
	if (0 <= row && row < fUEOIndices.EIndex && 0 <= column && column < fUEOIndices.UIndex)
		globalFirstDesignMatrix->coeffRef(row,column) = coeff;
	} catch(...) {
		return false;
	}
	return true;
...
{
	try {
		if (0 <= row && row < fUEOIndices.WIndex && 0 <= column && column < fUEOIndices.UIndex)
			globalFirstDesignMatrix->coeffRef(row + fUEOIndices.EIndex, column) = coeff;
	} catch(...) {
		return false;
	}
	return true;
...
bool TLSInputMatrices::setSecondDgnMtrxBlock(MatrixIndex firstIndex, MatrixIndex secondIndex, Eigen::MatrixXd block)
{
	try
	{
		...
		for (int row = 0; row < block.rows(); row++)
		{
			for (int col = 0; col < block.cols(); col++)
			{
				globalSecondDesignMatrix->insert(firstIndex + row, secondIndex + col) = block(row, col);
			}
		}
		...
		if ((block.cols() == block.rows() && firstIndex == secondIndex && secondDesignMatrixIsBlockDiag))
		{
			...
			for (int row = 0; row < dim; row++)
			{
				...
				{
					globalSecondDesignBlockDiagInvMatrix->insert(firstIndex + row, secondIndex + col) = block_inverse(row, col);
				}
			}
		}
		else
...
{
	try {
		(*fGlobalMisclosureVector)(row) = coeff;
	} catch(...) {
		return false;
	}
	return true;
...
{
	try {
		(*fGlobalMisclosureVector)(row + fUEOIndices.EIndex) = coeff;
	} catch(...) {
		return false;
	}
	return true;
...
bool TLSInputMatrices::setWeightMtrxElement(MatrixIndex row, MatrixIndex column, TReal coeff)
{
	...
	if (0 <= row && row < fUEOIndices.OIndex && 0 <= column && column < fUEOIndices.OIndex){
		globalWeightMatrix->insert(row,column) = coeff;
	}
...
bool TLSInputMatrices::setWeightInvMtrxElement(MatrixIndex row, MatrixIndex column, TReal coeff)
{
	try {
	if (0 <= row && row < fUEOIndices.OIndex && 0 <= column && column < fUEOIndices.OIndex)
		globalWeightInvMatrix->insert(row,column) = coeff;
	} catch(...) {
		return false;
	}
	return true;
...
{
	try {
		if (0 <= row && row < fUEOIndices.WIndex && 0 <= column && column < fUEOIndices.WIndex)
			globalWeightMatrix->insert(row + fUEOIndices.OIndex, column + fUEOIndices.OIndex) = coeff;
	} catch(...) {
		return false;
	}
	return true;
...
bool TLSInputMatrices::setWeightUnkInvMtrxElement(MatrixIndex row, MatrixIndex column, TReal coeff)
{
	try {
		if (0 <= row && row < fUEOIndices.WIndex && 0 <= column && column < fUEOIndices.WIndex)
			globalWeightInvMatrix->insert(row + fUEOIndices.OIndex, column + fUEOIndices.OIndex) = coeff;
	} catch(...) {
		return false;
	}
	return true;
